...
for (let i = 0; i < bigButtons.length; i++) {
  bigButtons[i].addEventListener('click', (e) => {   
    ...
    const checkForProjectPageChange = () => {
      if (e.currentTarget === projectBtn) loadProjectPage()
      return null;
    }
    ...
    if ( e.currentTarget === projectBtn ) { // Setup project page reload for *after* transitions ends
    ...
    if ( e.currentTarget === blogBtn ) { // Main blog content fly in from left animation 
